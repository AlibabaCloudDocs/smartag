...
-   Endpoint is the entry of the Alibaba Cloud service to call. The end point of Smart Access Gateway is smartag.cn-shanghai.aliyuncs.com.
-   Action is the action to perform. For example, the DescribeSmartAccessGateways API can be used to query created Smart Access Gateway instances.
-   Version is the version of the VPI. The version of the Smart Access Gateway APIs is 2018-03-13.
...
For security purposes, we recommend that you allow a RAM user to call APIs. Before a RAM user can start to call APIs, you must grant the RAM user permission by creating an authorization policy and attaching the policy to the RAM user. For more information, see [RAM authentication](intl.en-US/API Reference/RAM authentication.md#).
...
Smart Access Gateway uses AccessKeyId and AccessKeySecret for symmetric encryption to verify the identity of the caller. AccessKey is a type of credential \(similar to password\) for Alibaba Cloud accounts and RAM users, where the AccessKeyId identifies the callers and the AccessKeySecret is a secret key for encrypting verifying the signature string. The AccessKeySecret must be kept confidential.
